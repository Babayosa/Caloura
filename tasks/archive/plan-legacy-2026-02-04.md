...
| Date | Change |
|------|--------|
| 2026-02-04 | **Recovery Refactor: No-Keychain Runtime + File-Backed Encrypted History.** Removed runtime keychain dependency for license/history flows, added silent best-effort legacy keychain migration for old license keys, moved encrypted history storage to `Application Support/Caloura/history.enc` with local AES key file permissions (`700` dir, `600` file), removed interactive unlock gates from License/History UI, added capture + launch timing instrumentation, thumbnail cache for History grid, and CI release guard workflow with explicit tag/version checks. |
...
## Next-Day Focus (2026-02-05)

- Verify public download button fetches `Caloura-1.0.5.zip` and the file opens without Gatekeeper warnings.
- Test Sparkle update path from 1.0.4 → 1.0.5 (auto-update and manual “Check for Updates”).
- Update Gumroad download file to 1.0.5 (match the appcast + website).
- Replace placeholder appcast release notes for 1.0.5 with final bullets.
- Optional: clean-machine install verification on macOS 14.x if a Sonoma VM is available.
