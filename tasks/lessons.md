...
### 2026-07-02 — A local gate that predicts CI must match CI's *strictness*, not just its checks [domain: swift|process|ci]
**Mistake:** A new test file passed the pre-commit hook (per-file `swiftlint lint --quiet`, non-strict) but carried two 122/124-char `line_length` violations that CI's project-wide `swiftlint --strict` fails on. The hook exists to catch CI failures early, yet here it would have waved a red-on-CI commit straight through.
**Root cause:** `line_length` (and most default rules) have severity `warning`; plain `swiftlint lint` exits 0 on warnings — only `--strict` promotes them to errors. The hook ran non-strict while CI ran `--strict`, so it was strictly weaker on the exact dimension it was meant to guard. Caught only because I reproduced CI's `--strict` locally before pushing.
**Rule:** A local gate meant to predict CI must run the same *failure threshold* as CI for every check it shares (`swiftlint --quiet --strict`), or it produces false-green and lulls you into pushing red. It may cover *fewer* checks than CI (a subset by scope is fine), but must never be *weaker* on a check it does run. Applied: `.githooks/pre-commit` now runs `--strict` per staged file.
**Confidence:** medium (2x — same CI-vs-local-drift class as the pinned-xcodegen and isKeyWindow lessons)

### 2026-07-02 — Refinement: `isKeyWindow` system tests DO pass under a *detached* `xcodebuild test` — only a focused-foreground Terminal starves them [domain: swift|process|ci]
**Updates** the 2026-07-01 isKeyWindow entry, whose pessimistic tail ("treat full-suite key-window results as CI's job") is too strong. Re-ran the full `xcodebuild test` as a *backgrounded* process (not attached to a frontmost Terminal) and all 4 `CalouraSystemTests` methods **passed** — `overlayWindows.filter(\.isKeyWindow).count` was correct — twice, plus the whole 820-test suite green.
**Root cause of the difference:** `isKeyWindow` needs the owning app frontmost. Launched detached with nothing else holding focus in the GUI session, the xctest host becomes active and the overlay panel becomes key — identical to a GHA runner. The earlier 5 assertion failures came specifically from launching `xcodebuild` in a Terminal that stayed frontmost and kept key status.
**Rule:** Local observation of frontmost-dependent system tests IS achievable — run `xcodebuild test` detached (background task / `nohup`; don't keep a focused Terminal in front), or on a machine where nothing competes for focus. Don't assert "system tests can only be verified on CI"; that's only true for a focused-foreground CLI run. Still never weaken the assertion — if it fails, first check focus, then diff the code.
**Confidence:** medium (supersedes the pessimistic tail of the 2026-07-01 entry)
